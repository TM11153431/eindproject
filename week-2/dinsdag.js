...
function initialize(error, results) {
	...
	var components = [ choropleth(features, data), scatterplot(onBrush, data1) ]
    ...
    function onBrush(x0, x1, y0, y1) {
        ...
        data.forEach(function (d) {
            d.filtered = clear ? false
                : d.Global_emissions < x0 || d.Global_emissions > x1 || d.renewables < y0 || d.renewables > y1
        })
        update()
    }
    update()
...
function filterXas(data, Xas) {
  	var Xas = {};
  	data.forEach(function(d){ Xas[d.Country] = +d.Xas })
  		
  	return Xas;
}  	

function scatterplot(onBrush, data1) {


	console.log(data1)
	//var result = []
	//data.forEach(function(d){ result[d.Country] = +d.hpi })
	//console.log(result)
	
	//console.log(hpi)
	//var belle = filterXas(data1, hpi)
	//console.log(belle)
    ...
    return function update(data) {
        x.domain(d3.extent(data, function (d) { return d.Global_emissions })).nice()
        y.domain(d3.extent(data, function (d) { return d.renewables })).nice()
    ...
    circle.enter().append('circle')
        ...
        .style('stroke', '#fff')
        .merge(circle)
        .attr('cx', function (d) { return x(d.Global_emissions) })
