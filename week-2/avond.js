...
// load the files
d3.queue()
    .defer(d3.csv, 'Global_emissions.csv', function (d) {
        return {
            name: d["country"],
            global_e: +d["global_emissions"],
            //status: +d["status"]
		
        }
    })
    .defer(d3.json, 'hp_index.csv')
...
function choropleth(features, data) {
	...
	var dataGE = {};
  	data.forEach(function(d) { dataGE[d.country] = +d.global_em; });
  	
  	// pair state name with state id
  	var landGE = {};
  	data.forEach(function(d) { landGE[d.Country] = d.Country; });
        ...
        // mousover
      	//.on("mouseover", function(d) {
        	...
        	//sel.moveToFront();
      		...
      		//.style("opacity", 1) 
  			//div.html(landHPI[d.name] + ":" + dataHPI[d.name] + 
  			//	 dataWB[d.name] +
  			//	dataLE[d.name] + dataIO[d.name] + dataEF[d.name])
    ...
    return function update(data) {
        svg.selectAll('path')
            ...
            .style('fill', function (d) { return d.filtered ? '#ddd' : color(d.global_em) })  
    }  
}
